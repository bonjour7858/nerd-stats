...
async function fetchVideoData(url) {
  ...
  if (!videoId) throw new Error("Format d'URL invalide.");
  ...
  if (!response.ok) throw new Error("Erreur de réponse de l'API YouTube.");

  const data = await response.json();
  if (!data.items || data.items.length === 0) throw new Error("Vidéo introuvable.");
...
function fetchMockData(videoId) {
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve({
        id: videoId,
        title: "Analyse Vidéo — Démo NerdStats",
